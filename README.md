...
在多轮对话中，当工具可用性发生变化时，Agent 会自动更新工具绑定状态：

```typescript
// 检查工具绑定状态
const bindingStatus = agent.checkToolBindingStatus();
console.log('绑定状态:', bindingStatus);
// 输出: { boundTools: [...], availableTools: [...], needsRebind: boolean }

// 手动触发工具重新绑定
agent.triggerToolRebind();

// 监听工具状态变化
agent.onToolExecutionEvent('tool.availability.changed', (event) => {
  console.log('工具可用性变化:', event.data);
});
```
...
# 运行动态工具绑定演示
npx tsx examples/dynamic-tool-binding-demo.mts
